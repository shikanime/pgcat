...
impl AsyncWrite for StreamInner {
    fn poll_write(
        ...
        match this {
            ...
            SteamInnerProj::Unix { stream } => stream.poll_write(cx, buf),
        }
    }
    ...
    fn poll_flush(
        ...
        match this {
            ...
            SteamInnerProj::Unix { stream } => stream.poll_flush(cx),
        }
    }
    ...
    fn poll_shutdown(
        ...
        match this {
            ...
            SteamInnerProj::Unix { stream } => stream.poll_shutdown(cx),
        }
    }
}
...
impl AsyncRead for StreamInner {
    fn poll_read(
        ...
        match this {
            ...
            SteamInnerProj::Unix { stream } => stream.poll_read(cx, buf),
        }
    }
}
...
impl Server {
    ...
    pub async fn startup(
        ...
        let mut stream = if address.host.starts_with('/') {
            startup_unix_stream(address).await?
        } else {
            startup_tcp_stream(address).await?
        };
...
async fn startup_tcp_stream(address: &Address) -> Result<StreamInner, Error> {
...
async fn startup_unix_stream(address: &Address) -> Result<StreamInner, Error> {
